...
import {
  ...
  DollarSign, Clock, ChevronRight, CreditCard, Loader2
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { STRIPE_PLANS, ALACARTE_ITEMS, type AlacarteKey, type PlanTier } from '@shared/stripeProducts';
...
const PLAN_META: Record<PlanTier, {
  ...
  free: {
    icon: Zap, color: 'slate', tagline: 'Get organized, no commitment', cta: 'Start Free',
    locked: ['TaxGPT AI assistant', 'Unlimited receipts & mileage', 'Bank account sync', 'E-filing'],
  },
  essential: {
    icon: Receipt, color: 'blue', tagline: 'For gig workers & side hustlers', cta: 'Get Essential',
    locked: ['Crypto tax tracking', 'Business entity management', 'Audit defense hub', 'Full Academy access'],
  },
  pro: {
    icon: Star, color: 'emerald', tagline: 'For freelancers & self-employed', cta: 'Go Pro', popular: true,
    locked: [],
  },
  business: {
    icon: Building2, color: 'violet', tagline: 'For small businesses & multi-entity', cta: 'Get Business',
    locked: [],
  },
};

const ADDONS = [
  { key: 'remoteReturn' as AlacarteKey, icon: Users, title: 'Remote Return — Human Filed', desc: 'Upload your documents and our team prepares and e-files your return. Guaranteed accurate.', price: '$199', badge: 'Most Popular', badgeColor: 'emerald', path: '/remote-returns' },
  { key: 'stateReturn' as AlacarteKey, icon: MapPin, title: 'State Return Add-On', desc: 'Add a state return to your federal filing. We handle the forms, you just review and sign.', price: '$49', badge: null, badgeColor: null, path: '/remote-returns' },
  { key: 'notarySession' as AlacarteKey, icon: Shield, title: 'Mobile & In-Person Notary', desc: 'Local notarization — we come to you or meet in person. Business formation, contracts, real estate & more. RON coming soon.', price: '$15+', badge: 'Local', badgeColor: 'emerald', path: '/notary' },
  { key: 'auditDefense' as AlacarteKey, icon: Shield, title: 'Audit Defense Package', desc: "If the IRS comes knocking, we've got your back. Full representation and correspondence handling.", price: '$99/yr', badge: 'Peace of Mind', badgeColor: 'blue', path: '/audit-defense' },
  { key: 'bookkeepingCleanup' as AlacarteKey, icon: DollarSign, title: 'Bookkeeping Cleanup', desc: "Behind on your books? We'll clean up your records and get you current — one-time service.", price: '$149', badge: null, badgeColor: null, path: '/remote-returns' },
  { key: 'priorYearReturn' as AlacarteKey, icon: FileText, title: 'Prior Year Return', desc: 'Need to file a previous year? We handle late returns for any year, no judgment.', price: '$199', badge: null, badgeColor: null, path: '/remote-returns' },
];
...
export default function Pricing() {
  ...
  const createAlacarteCheckout = trpc.billing.createAlacarteCheckout.useMutation({
    onSuccess: (data) => {
      if (data.url) {
        toast.info('Redirecting to secure checkout...');
        window.open(data.url, '_blank');
  ...
  const handleSubscribe = (tier: PlanTier, priceId: string | null) => {
    if (tier === 'free') {
    ...
    createCheckout.mutate({ priceId, origin: window.location.origin });
  };

  const handleAlacarte = (key: AlacarteKey) => {
    if (!isAuthenticated) {
      window.location.href = getLoginUrl();
      return;
    }
    createAlacarteCheckout.mutate({ itemKey: key, origin: window.location.origin });
  };
      ...
      <div className="max-w-6xl mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
          {STRIPE_PLANS.map((plan) => {
            ...
            const displayPrice = plan.monthlyAmount === 0
              ? 'Free'
              : isAnnual
              ? `$${(plan.annualAmount / 100).toFixed(0)}`
              : `$${(plan.monthlyAmount / 100).toFixed(2)}`;
            const period = plan.monthlyAmount === 0 ? 'forever' : isAnnual ? '/yr' : '/mo';
            const savings = isAnnual && plan.monthlyAmount > 0
              ? `Save $${((plan.monthlyAmount * 12 - plan.annualAmount) / 100).toFixed(0)} vs monthly`
              : null;
            ...
            return (
              <div key={plan.tier} className={cn('relative bg-slate-900 border rounded-2xl p-6 flex flex-col', styles.ring, meta.popular ? 'shadow-lg shadow-emerald-500/10' : '')}>
                ...
                <Button
                  className={cn('w-full mb-6', styles.btn)}
                  onClick={() => handleSubscribe(plan.tier, priceId)}
                  disabled={isPending}
                >
                  {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <>{meta.cta} <ArrowRight className="w-4 h-4 ml-2" /></>}
                </Button>
